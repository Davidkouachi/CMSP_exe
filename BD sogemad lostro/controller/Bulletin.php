...
class Bulletin extends Admin_Controller {
    ...
    function __construct()
    {
	 ...
	 public function RecuBulPrint($numfac)
	{
			$date = date('Y-m-d');

			$infoTestPatient = $this->printmodel->get_info_test_patient($numfac,$date);
			...
			$page_data['print_name'] = 'i_RB';
			$page_data['infos_labo'] = $infoTestPatient;
			...
			// Effectuer la journalisation
		        ...
		        $action_effectuee = 'Reçu de biologie/imagerie' ;
    ...
    public function BulletinRegister()
    {
		if(empty($_POST))
		{	
		 $UniqueProfil = $this->UserModel->get_UniqueProfil($this->session->userdata('user_name'));
			...
			$page_data['page_name'] = 'BulletinRegister';
			$page_data['page_active'] = 'CaissePage';
			$page_data['page_profil'] = $this->session->userdata('user_profil');
			$page_data['page_title'] = 'Lostro Admin';
			$page_data['page_s_title'] = 'ENCAISSER UNE FACTURE D\'IMAGERIE OU DE BIOLOGIE';
			
			$page_data['solde_caisse'] = $this->all_model->get_solde_caisse(date('Y-m-d')) ;
			
			$page_data['lien'] = base_url() . 'ajax/ouverture_caisse/Bulletin' ;

			// Effectuer la journalisation
		        $type_action = 'Consultation' ;

		        $action_effectuee = 'Formulaire d\'encaissement de facture imagerie/biologie' ;

		        $this->control->journalisation($type_action,$action_effectuee) ;
			
			// affichage de la vue

			$this->render_template('kameleon/BulletinRegister', $page_data);
		}
    }

    public function ReglerFactureBul()
    { 
		...
		if(!empty($_POST))
		{
            ...
            $this->form_validation->set_rules('numfac', 'Numéro de facture', 'trim|required|alpha_dash|min_length[9]|max_length[9]');
            ...
            else
            {
				...
				if($deb_facture != 'FCB' )
				{
					echo '<div role="alert" class="alert alert-danger">
	                        <button data-dismiss="alert" class="close" type="button"><span aria-hidden="true">x</span><span class="sr-only">Close</span></button>
	                        Le numéro saisi n\'est pas celui d\'une facture de biologie/imagerie. Veuillez renseigner un numéro de facture de biologie/imagerie valide.
	                    </div>';
	                    exit();
				}
				
				
				//$bulletin_regle = $this->db->get_where('factures' , array('numfac'=>$numfac))->row_array();
				...
				if(!empty($facture))
				{
					...
					//$montanttotal = $bulletin_regle['montanttotal'];
				...
				else
				{
					...
					echo '<div role="alert" class="alert alert-danger">
	                        <button data-dismiss="alert" class="close" type="button"><span aria-hidden="true">x</span><span class="sr-only">Close</span></button>
	                        Il n\'existe aucune facture de biologie/imagerie à reglé pour le numéro que vous avez saisi. Veuillez renseigner un numéro de facture de biologie/imagerie valide.
	                    </div>';
	                    exit();
				}
				...
				//$ticketmod = $montanttotal * ((100-$tauxpatient['valeurtaux'])/100);
			   ...
			   if($mois_cpteur == $mois)
			    ...
			    $this->all_model->set_compteur($compteur) ;
				  ...
				  // Effectuer la journalisation
			        ...
			        $action_effectuee = 'Encaissement de facture biologie/imagerie' ;
				...
				else
				{
				    ...
				    // Effectuer la journalisation
				        ...
				        $action_effectuee = 'Informations de facture biologie/imagerie' ;
				    ...
				    <div class="col-md-12">
				      ...
				      <div class="col-md-10">
				        <div class="form-group">
				          <div class="form-group">
				            <input type="text" style="color:black; font-weight:bold; font-size:25px;" readonly class="form-control" id="" value="<?php echo strtoupper($patient['nomprenomspatient'])  ?>">
				          </div>
				        </div>
				      </div>
				    </div>
